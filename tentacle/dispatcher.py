...
class EventDispatcher(object):
    ...
    def format_payload(self, worker_type, payload):
    	...
    	celery_standard = ['task', 'id', 'args', 'kwargs', 'retries', 'eta', 'expires']

    	fixed_payload = dict(item for item in payload.items() if item[0] in celery_standard)
    	#check the format and reject messages that are not correct
    	if worker_type == 'kraken' and fixed_payload['task'] != 'kraken.tasks.RPCTask':
    		raise TypeError('Kraken task does not have the correct attributes.')
    	if worker_type == 'nautilus' and 'method' in fixed_payload['kwargs']:
    ...
    def dispatch(self, task):
    	...
    	worker_type = task.pop('worker')
    	worker_connection = getattr(self, '{}_conn'.format(worker_type))
    	exchange = Exchange(worker_type)
